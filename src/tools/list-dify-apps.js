...
exports.default = listDifyAppsTool;
 ...
 */
async function findBestMatchingAppWithAI(query, apps) {
    ...
    `;
        ...
        const aiResponse = await api_1.AI.ask(prompt, {
        ...
        const matchedApp = apps.find(app => app.name.toLowerCase() === appName.toLowerCase() ||
 ...
 */
function findBestMatchingAppTraditional(query, apps) {
    if (!apps || apps.length === 0)
        return null;
    ...
    const appsWithScores = apps.map(app => {
        ...
        // Inputs matching
        const inputKeys = Object.keys(app.inputs || {}).join(" ").toLowerCase();
...
function parseUserInput(query) {
    ...
    const timePeriodPattern = /(?:apps|dify apps)\s+(?:from|created|added|made)\s+(?:in|during|within)?\s+(?:the\s+)?(?:last|past)\s+(\d+)\s+(day|days|week|weeks|month|months)/i;
    ...
    const timeAgoPattern = /(?:apps|dify apps)\s+(?:created|added|made|from)\s+(\d+)\s+(day|days|week|weeks|month|months)\s+ago/i;
 ...
 */
function filterAppsByTime(apps, timeFilter) {
    if (!apps || apps.length === 0)
        return [];
 ...
 */
function formatInputRequirements(app) {
 ...
 */
function generateAppDescription(app) {
    if (app.description) {
 ...
 */
async function listDifyAppsTool(input) {
    ...
    try {
        ...
        if (!input.query || input.query.trim() === "") {
            ...
            return JSON.stringify({
                message: "Please provide a query about Dify applications. You can ask to list your apps or find a specific app."
            });
        }
        // Parse user input
        ...
        if (searchTerm === "list_all") {
            ...
            if (matchedApp) {
                ...
                return JSON.stringify({
                    ...
                    app: formattedApp,
                    found: true,
                    match_criteria: "Your query matched this application based on its name, endpoint, description, and other properties."
                });
            }
            else {
